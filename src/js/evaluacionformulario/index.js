...
const CargarDatosEvaluado = async () => {
    const catalogoEvaluado = new URLSearchParams(window.location.search).get('catalogo');
...
const CargarMeritos = async (nota) => {
    ...
    try {
        ...
        if (codigo === 1) {
            ...
            const select = nota === 3 ? document.getElementById('merito_1') : document.getElementById('merito_2');
            if (select) {
                select.innerHTML = '<option value="0">No hay méritos disponibles</option>';
            }
        }
...
// EVENT LISTENERS PARA PERFIL BIOFÍSICO
const perfilRadios = document.querySelectorAll('input[name="bol_perfil"]');
